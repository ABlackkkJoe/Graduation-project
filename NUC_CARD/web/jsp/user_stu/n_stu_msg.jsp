...
<div class="right">
      ...
      <table class="providerTable" cellpadding="0" cellspacing="0">
		  ...
		  <c:forEach var="consume" items="${consumes}" varStatus="status">
			  <tr>
				  <td>
					  <c:if test="${consume.acccode ==210}"><span>餐厅</span></c:if>
					  <c:if test="${consume.acccode == 211}"><span>淋浴</span></c:if>
					  <c:if test="${consume.acccode == 222}"><span>饮水</span></c:if>
				  </td>
					<td>
						<span>${consume.opdt}</span>
					</td>
					<td>
						<span>${consume.opfar}</span>
					</td>
					<td>
