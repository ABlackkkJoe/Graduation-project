...
public class UserServlet extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (null != method && method.equals("add")) {                   // 添加操作
            ...
            this.queryStuName(request, response);
        }else if (null != method && method.equals("queryStudent")) {          // 查询学生操作
            this.queryStudent(request, response);
        }else if (null != method && method.equals("dp")) {          // 级联查询操作
                this.dp(request, response);
    ...
    private void queryStudent(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String queryUserName = request.getParameter("queryname");
        System.out.println(queryUserName);
        queryUserName = new String(queryUserName.getBytes("ISO-8859-1"),"UTF-8");
        String temp = request.getParameter("queryUserRole");
        String pageIndex = request.getParameter("pageIndex");
        int queryUserRole = 0;
        int currentPageNo = 1;
    ...
    private void queryStu(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        // 查询用户列表
        String queryUserName = request.getParameter("queryname");
        System.out.println(queryUserName);
        String temp = request.getParameter("queryUserRole");
    ...
    private void queryStuName(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        // 查询用户列表
        String queryUserName = request.getParameter("queryname");
        queryUserName = new String(queryUserName.getBytes("ISO-8859-1"),"UTF-8");
        System.out.println(queryUserName);
    ...
    private void dp(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //年级
        if(!school.isEmpty() && major.equals("请选择专业") && num.equals("请选择班级")){
            request.getRequestDispatcher("/jsp/user_dp/dpShow_1.jsp").forward(request, response);
        }
        //专业
        if(!school.isEmpty() && !major.isEmpty() && num.equals("请选择班级")){
            request.getRequestDispatcher("/jsp/user_dp/dpShow_2.jsp").forward(request, response);
        }
        //班级
        if(!school.isEmpty() && !major.isEmpty() && !num.isEmpty()){
            request.getRequestDispatcher("/jsp/user_dp/dpShow_3.jsp").forward(request, response);
        }
