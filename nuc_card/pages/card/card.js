...
Page({
  data: {
    ...
    income:0,
    balance: getApp().globalData.balance,        // 当前余额（余额卡片上的展示数据）
    last_time: '',
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if (res.statusCode == 200) {
          var data = res.data.slice(0, _this.data.count).reverse()
         // var data = res.data.slice(0, _this.data.count);
          /*获取最近消费数据绘制折线图*/
          var dict = data;
          var len = dict.length,
            xArr = [],           // x轴坐标
          ...
          _this.setData({
            ...
            last_time: data[data.length - 1].opedt,
            userCode: getApp().globalData.userCode,
          ...
          getApp().globalData.balance = _this.data.balance;
          //canvas 
          ...
          */
          ...
          _this.setData({
            remind: res.data.message || '请求错误，检查url'
          });
        }
      },
      fail: function (res) {
      ...
      success: function (res) {
        ...
        wx.request({
          ...
          success: function (res) {
            ...
            var rs=res.data[0].balance-opfare
            var t=res.data[0].opedt
            if (rs>0||rs==0){
              wx.showModal({
                title: '消费成功',
                content: +opfare + "元",
               ...
               _this.setData({
                 balance:_this.data.balance-opfare,
                 last_time:t
               }),
                getApp().globalData.balance = _this.data.balance;
            }
            if(rs<0){wx.showModal({
                title: '失败',
                content: '余额不足！',
              })
            }
  ...
  recharge: function () {
    var _this = this;
    wx.navigateTo({
      url: '../pay/pay?balance=' + getApp().globalData.balance,
    })
  },
  // 绘制横轴&纵轴&网格线
